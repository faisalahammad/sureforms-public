...
class ActionScheduler_Compatibility {
	...
	public static function raise_memory_limit() {
		...
		$filtered_limit     = apply_filters( 'admin_memory_limit', $wp_max_limit ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals -- third-party core hook from WP
		$filtered_limit_int = self::convert_hr_to_bytes( $filtered_limit );

		if ( -1 === $filtered_limit_int || ( $filtered_limit_int > $wp_max_limit_int && $filtered_limit_int > $current_limit_int ) ) {
			if ( false !== @ini_set( 'memory_limit', $filtered_limit ) ) { // phpcs:ignore Squiz.PHP.DiscouragedFunctions -- vendored lib, required to raise memory limit
				return $filtered_limit;
			} else {
				return false;
			}
		} elseif ( -1 === $wp_max_limit_int || $wp_max_limit_int > $current_limit_int ) {
			if ( false !== @ini_set( 'memory_limit', $wp_max_limit ) ) { // phpcs:ignore Squiz.PHP.DiscouragedFunctions -- vendored lib, required to raise memory limit
				return $wp_max_limit;
			} else {
				return false;
			}
	...
	public static function raise_time_limit( $limit = 0 ) {
		...
		if ( function_exists( 'wc_set_time_limit' ) ) {
			...
			@set_time_limit( $raise_by ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged,Squiz.PHP.DiscouragedFunctions -- vendored lib, required to raise time limit
		}
	}
}
