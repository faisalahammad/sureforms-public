...
class Test_Global_Settings extends TestCase {
	...
	/**
	 * The Abilities API saves general settings as a partial array that never
	 * carries srfm_distraction_free. That save must keep the stored value
	 * instead of switching Distraction Free off. An explicit value still wins.
	 */
	public function test_srfm_save_general_settings_keeps_distraction_free_when_key_absent() {
		$backup = get_option( 'srfm_general_settings_options', [] );

		try {
			\SRFM\Inc\Global_Settings\Global_Settings::srfm_save_general_settings( [ 'srfm_distraction_free' => true ] );
			$this->assertTrue( get_option( 'srfm_general_settings_options' )['srfm_distraction_free'] );

			\SRFM\Inc\Global_Settings\Global_Settings::srfm_save_general_settings( [ 'srfm_ip_log' => true ] );
			$this->assertTrue(
				get_option( 'srfm_general_settings_options' )['srfm_distraction_free'],
				'A save without the key must keep Distraction Free on.'
			);

			\SRFM\Inc\Global_Settings\Global_Settings::srfm_save_general_settings( [ 'srfm_distraction_free' => false ] );
			$this->assertFalse( get_option( 'srfm_general_settings_options' )['srfm_distraction_free'] );
		} finally {
			update_option( 'srfm_general_settings_options', $backup );
		}
	}
	...
	/**
	 * The Settings UI must always receive srfm_distraction_free, defaulting to off,
	 * both on a fresh install and when the stored option predates the setting.
	 */
	public function test_srfm_get_general_settings_backfills_distraction_free() {
		$backup  = get_option( 'srfm_general_settings_options', [] );
		...
		try {
			...
			foreach ( [ 'fresh install' => null, 'older option' => [ 'srfm_ip_log' => true ] ] as $case => $stored ) {
				...
				$response = \SRFM\Inc\Global_Settings\Global_Settings::srfm_get_general_settings( $request );
				$data     = $response instanceof \WP_REST_Response ? $response->get_data() : $response;

				$this->assertArrayHasKey( 'srfm_distraction_free', $data['srfm_general_settings_options'], $case );
				$this->assertFalse( $data['srfm_general_settings_options']['srfm_distraction_free'], $case );
			}
		} finally {
			update_option( 'srfm_general_settings_options', $backup );
		}
